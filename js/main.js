...
const ABSCHNITT_STROKE_COLOR = '#4891c2';
...
const NETZKNOTEN_SIGN_CONTENT_PAD_Y = 2;
const NETZKNOTEN_SIGN_MIN_WIDTH = 60;
const NETZKNOTEN_SIGN_MIN_HEIGHT = 24;
...
function buildSvgFontDefs() {
  const parts = [];
  if (svgEmbedFontRegular)
    parts.push(`@font-face{font-family:'ddin-regular';src:url('${svgEmbedFontRegular}') format('woff2');}`);
  if (svgEmbedFontBold)
...
function createNetzknotenCompactKtSignSvg({ ktText, blue = true }) {
  const signBlue = blue ? ABSCHNITT_STROKE_COLOR : '#f5f7fa';
  ...
  return `
    <div class="tblOption tblOption--ast tblOption--astForeign">
      ...
      <div class="tblCell tblCell--astAoa tblCell--astAoa--foreign">
        <div class="${blueSignClass}">
          <div class="blueSignContent">
            ${renderInlineBabShield(data.ast_bab)}
            ${iconHtml}
            ${pillHtml}
